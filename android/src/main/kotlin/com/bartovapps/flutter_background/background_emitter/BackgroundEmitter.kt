...
import com.bartovapps.flutter_background.FlutterBackgroundPlugin.Companion.ARG_APP_CALLBACK_HANDLE_
        ...
        if(ready){
            handler.post {
                try {
                    ...
                    val params = mapOf(ARG_APP_CALLBACK_HANDLE_ to appRawHandle, MESSAGE to event)
                    internalMethodChannel?.invokeMethod("FlutterBackground#BackgroundMessage", params)
                } catch (e: Throwable){
                    Log.e(LOG_TAG, "Something went wrong: ${e.message}")
                }
            }
        } else {
...
//    private fun <T> runAsync(operation: () -> T?) : Result<T?> {
//        handler.post {
//            runCatching {
//                operation.invoke()
//            }.onSuccess {
